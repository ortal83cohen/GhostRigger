...
class PieceSelector extends DeviceModuleBase {
  ...
  Future<void> load() async {
    sprite = await Sprite.load('module_selection_area.png');
    super.load();
  }
  ...
  void tryToAddPiece(Piece piece, Offset dragPosition) {
    if (area!.contains(dragPosition) && !pieces.contains(piece)) {
      hackingDevice.board.pieces.forEach((boardPieces) {
        ...
        if (pieceIndex != -1) boardPieces[pieceIndex] = null;
      });
      pieces.remove(piece);
      pieces.add(piece);
  ...
  void render(Canvas canvas) {
    ...
    pieces.toList().forEach((piece) {
      var pieceOffsetY =
          offsetY + (pieceIndex * hackingDevice.board.cellSize) + (spacingBetweenPieces * (pieceIndex + 1));
  ...
  void update(double t) {
    ...
    if (!isScrollValid) correctScroll(scroll);
  }
