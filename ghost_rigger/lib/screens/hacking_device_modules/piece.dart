...
class Piece extends DeviceModuleBase {
  ...
  late Sprite borderHighlightedSprite;
  ...
  Future<void> load() async {
    sprite = await Sprite.load(getSpriteName());
    borderSprite = await Sprite.load('line_border.png');
    borderHighlightedSprite = await Sprite.load('line_in_out.png');
    super.load();
  }

  String getSpriteName() {
    if (isInOrOut!) return 'line_in_out.png';
  ...
  void executeRender(Canvas canvas) {
    ...
    if (!isInOrOut!) {
      ...
      else if (draggedPiece == this) borderHighlightedSprite.renderRect(canvas, area!);
      if (arithmeticValue != 0) {
      ...
      var text = arithmeticValue != 0 ? '$arithmeticValue' : 'OUT';
      var textStyle = TextStyle(color: Colors.white, fontFamily: 'Rajdhani', fontSize: 18);
      var textSpan = TextSpan(text: text, style: textStyle);
      var textPainter = TextPainter(text: textSpan, textDirection: TextDirection.ltr);
      var extraOffsetX = arithmeticValue != 0 ? area!.width * 0.1 : -area!.width * 0.05;
      var offset = Offset(area!.left + (area!.width * 0.32) + extraOffsetX, area!.top + (area!.width * 0.31));
  ...
  void update(double t) {
    ...
    else if (distanceRatio < 1) deltaX *= distanceRatio;
  ...
  void executeDragging(double dX, double dY, {bool updatePosition = true}) {
    if (hackingDevice.pieceSelector.scrollDirection != 0 ||
        ...
        hackingDevice.isShowingInfo!) return;

    if (draggedPiece == this || area?.contains(Offset(dX, dY)) == true) {
      draggedPiece = this;
      if (updatePosition) dragPosition = Offset(dX, dY);
    } else
      stopDraggingIfNecessary();
  }
