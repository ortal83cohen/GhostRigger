...
class Info extends DeviceModuleBase {
  ...
  Future<void> load() async {
    backgroundSprite = await Sprite.load('info.png');
    super.load();
  }
  ...
  void setTitle(Canvas canvas) {
    ...
    var textPainter = TextPainter(text: textSpan, textDirection: TextDirection.ltr, textAlign: TextAlign.left);
    var offset = Offset(area!.left + (area!.height * 0.13), area!.top + (area!.height * 0.11));
    textPainter.layout(
        minWidth: area!.width - ((area!.height * 0.13) * 2), maxWidth: area!.width - ((area!.height * 0.13) * 2));
    textPainter.paint(canvas, offset);
  }

  void setText(Canvas canvas) {
    var textStyle = TextStyle(color: Colors.white, fontFamily: 'Rajdhani', fontSize: 17);
    var textSpan = TextSpan(text: text, style: textStyle);
    var textPainter = TextPainter(text: textSpan, textDirection: TextDirection.ltr, textAlign: TextAlign.left);
    var offset = Offset(area!.left + (area!.height * 0.13), area!.top + (area!.height * 0.27));
    textPainter.layout(
        minWidth: area!.width - ((area!.height * 0.13) * 2), maxWidth: area!.width - ((area!.height * 0.13) * 2));
    textPainter.paint(canvas, offset);
  }
