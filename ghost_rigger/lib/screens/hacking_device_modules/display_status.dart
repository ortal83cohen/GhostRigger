...
class DisplayStatus extends DeviceModuleBase {
  ...
  Future<void> load() async {
    sprite = await Sprite.load('display_steps_and_status.png');
    super.load();
  }
  ...
  void setCaseNumberText(Canvas canvas) {
    ...
    var textStyle = TextStyle(color: Color.fromARGB(255, 40, 192, 252), fontFamily: 'Rajdhani', fontSize: 20);
    ...
    var offset = Offset(area!.left + (area!.height * 0.02), area!.top + (area!.width * 0.4));
    textPainter.layout(minWidth: area!.width - area!.width * 0.07, maxWidth: area!.width - area!.width * 0.07);
  ...
  void setStatusText(Canvas canvas) {
    ...
    var textStyle = TextStyle(color: Color.fromARGB(255, 40, 192, 252), fontFamily: 'Rajdhani', fontSize: 17);
    var textSpan = TextSpan(text: statusText, style: textStyle);
    var textPainter = TextPainter(text: textSpan, textDirection: TextDirection.ltr, textAlign: TextAlign.center);
    var offset = Offset(area!.left + (area!.height * 0.02), area!.top + (area!.height * 0.73));
    textPainter.layout(minWidth: area!.width - area!.width * 0.07, maxWidth: area!.width - area!.width * 0.07);
    textPainter.paint(canvas, offset);
  }
